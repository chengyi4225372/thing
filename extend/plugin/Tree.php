...
class Tree
{
    ...
    {
        $str         = '';
        $childlistli = '<li class="@class"><a href="@url"><i class="@icon"></i> <span> @title</span></a> @childlist</li>';
        $childs      = $this->getChild($myid);
        if ($childs) {
            foreach ($childs as $value) {
                $id         = $value['id'];
                $sub_childs = $this->getChild($id);
                if ($deeplevel == 0 && (!$sub_childs || empty($sub_childs))) {
                    $itemtpls = $childlistli;
                } else {
                    $itemtpls = $itemtpl;
                }
                unset($value['child']);
                ...
                $nstr     = strtr($itemtpls, $value);
                $value    = array_merge($value, $bakvalue);

                $childdata = $this->getTreeMenu($id, $childlistli, $selectedids, $disabledids, $wraptag, '', $deeplevel + 1);
                $childlist = $childdata ? "<{$wraptag} {$wrapattr}>" . $childdata . "</{$wraptag}>" : "";
                $childlist = strtr($childlist, array('@class' => $childdata ? 'last' : ''));

                $value = array(
                    '@childlist' => $childlist,
                    '@url'       => $childdata || !isset($value['@url']) ? "javascript:;" : url('/' . trim($value['@url'], '/')),
                    '@addtabs'   => $childdata || !isset($value['@url']) ? "" : (stripos($value['@url'], "?") !== false ? "&" : "?"),
    ...
    * @return  array
    */
    public function getTreeCate($list) : array
    {
        $tree = array();
        foreach($list as $item){
            if(isset($item['is_item']) && $item['is_item'])
            {
                continue;
            }
            if(isset($list[$item['pid']])){
                $list[$item['pid']]['son'][] = &$list[$item['id']];
            }else{
                $tree[] = &$list[$item['id']];
            }
        }
